...
radio.config(channel = 8)       # Channel default 7
...
while True: 
    ...
    if microbit.button_a.is_pressed():                  # Check if button A is pressed
        ...
        radio.config(channel = 9)                       # Tuning to channel 8 (sending message to micro:bit 2)
        radio.send(sending)                             # Send the message
        microbit.display.scroll("Sending")          
        microbit.display.show(alphabet[myIndex])
        radio.config(channel = 8)                       # Tuning to channel 7 (ready for receiving messages from micro:bit 3)
