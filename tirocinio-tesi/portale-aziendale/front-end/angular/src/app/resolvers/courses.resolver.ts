...
export class CoursesResolver implements Resolve<CoursesData[]> {
    ...
    resolve(
        ...
        this.zone.runOutsideAngular(() => {
            setTimeout(() => {
                this.store.select(selectCoursesDataFilters).pipe(take(1)).subscribe((storedDqp) => {
                    this.store.dispatch(
                        searchCoursesData({
                            _id: rand,
                            queryParams: {
                                ...
                                ordering: storedDqp.ordering ? storedDqp.ordering : [{ column: "coursesId", columnPrefix: "", descending: false }],
                            },
                        })
                    );
                });
