...
public class ModificareTM extends Base {

	private static final long serialVersionUID = -5164142771079076286L;

	//	Attributi derivati da associazioni
	private GestireEasyCatalogo GEC;
//	fine associazioni
	
	private Id TipoDaModificare;  //  @jve:decl-index=0:

	private String EEdelTipo;  
	private InfoTipoMoneta tipo;
	private Id unita;  
	
	private Info InfoTipoMoneta;
	String[] unit;
...
//	<<create>> mkModificareTM(GestireEasyCatalogo gec,Id idM)
	public ModificareTM(GestireEasyCatalogo gec,Id idM){
		super();
		mystate=ATTESA_MODIFICA_EC;
		this.GEC=gec;
		this.TipoDaModificare=idM;
		
		GEC.modTM(this,TipoDaModificare);
	}
	...
	private void initialize() {
		...
		this.setTitle("EasyCoin - Modificare Tipo Moneta");
	...
	public void showEC(/*Visualizzazione*/Info ec){
		...
		case ATTESA_MODIFICA_EC:{
			mystate=ATTESA_CONFERMA_MOD_TM;
			/*Visualizza nell'interfaccia ec*/
			InfoTipoMoneta=ec;
			unita=((Tipo)InfoTipoMoneta.getTipoH().get(TipoDaModificare)).getUnita();
			tipo=((Tipo)InfoTipoMoneta.getTipoH().get(TipoDaModificare)).getInfoTipoMoneta();
			//System.out.print(((Tipo)InfoTipoMoneta.getTipoH().get(TipoDaModificare)));
			EEdelTipo=((EnteEmettitore)InfoTipoMoneta.getEnteEmettitoreH().get(((Tipo)InfoTipoMoneta.getTipoH().get(TipoDaModificare)).getEnteEmettitore().getId())).getInfoEE().getNome();
			un=GEC.getVIS().getPannelloUnita(ec.getUnitaH());
			int k=0;
			unit=new String[ec.getUnitaH().size()];
	...
	public void modificareTM(InfoTipoMoneta tipo,Id u){
		switch(mystate){
		...
		case ATTESA_DATI_TM:{
			mystate=MODIFICA_EC;
			setVisible(false);
			GEC.modificaTM(this, tipo, TipoDaModificare);
			break;} 
...
//	Metodi get e set
	...
	public Id getTipoDaModificare() {
		return TipoDaModificare;
	}

	public void setTipoDaModificare(Id idTM) {
		TipoDaModificare = idTM;
	}
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			jLTitolo.setText("Unità presenti per l'ente emettitore "+EEdelTipo);
	...
	private JTextField getJT_Descrizione() {
		if (jT_Descrizione == null) {
			...
			jT_Descrizione.setText(tipo.getDescrizione());
		}
	...
	private JTextField getJT_ParteIntera() {
		if (jT_ParteIntera == null) {
			...
			jT_ParteIntera.setText(tipo.getVnom().getParteIntera()+"");
		}
	...
	private JTextField getJT_Num() {
		if (jT_Num == null) {
			...
			jT_Num.setText(tipo.getVnom().getNumeratore()+"");
		}
		return jT_Num;
	}
	...
	private JTextField getJT_Den() {
		if (jT_Den == null) {
			...
			jT_Den.setText(tipo.getVnom().getDenominatore()+"");
		}
		return jT_Den;
	}
	...
	private JTextField getJT_Dimensione() {
		if (jT_Dimensione == null) {
			...
			jT_Dimensione.setText(tipo.getDimensione()+"");
		}
	...
	private JTextField getJT_Spessore() {
		if (jT_Spessore == null) {
			...
			jT_Spessore.setText(tipo.getSpessore()+"");
		}
		return jT_Spessore;
	}
	...
	private JTextField getJT_Bordo() {
		if (jT_Bordo == null) {
			...
			jT_Bordo.setText(tipo.getBordo());
		}
		return jT_Bordo;
	}
	...
	private JTextField getJT_Materiale() {
		if (jT_Materiale == null) {
			...
			jT_Materiale.setText(tipo.getMateriale());
		}
		return jT_Materiale;
	}
	...
	private JComboBox getJCB_Forma() {
		if (jCB_Forma == null) {
			...
			jCB_Forma.setSelectedItem(tipo.getForma().toString());
		}
		return jCB_Forma;
	}
		...
		if (jB_Conferma == null) {
			...
			jB_Conferma.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					System.out.println("actionPerformed() Conferma Modifica Tipo Moneta"); 
	...
	private JButton getJB_Annulla() {
		if (jB_Annulla == null) {
			...
			jB_Annulla.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					System.out.println("actionPerformed() Annulla Modifica Tipo Moneta");
					// TODO Auto-generated Event stub actionPerformed()
	...
	private JComboBox getJCB_Unita() {
		if (jCB_Unita == null) {
			...
			jCB_Unita.setSelectedItem(unita.idToString());
		}
		return jCB_Unita;
	}
