...
/*** TO BE COMPLETED START ***/
  ...
  printf("Solo dopo avergli assegnato un valore appropriato diverso da NULL, per esempio\n"
	...
	//se non ci sono argomenti or i==0 (c'è m) e non c'è "p"
  	sprintf(p,"%c!",c[2]);//compongo la stringa mettendo "o" al posto di %c!
	else{//ci sono argomenti (oltre al nome del programma) e c'è "p"
	  if(i==0)//se c'è "m"...
	    sprintf(p,"%c!",c[4]);//compongo la stringa mettendo "i" al posto di %c!
	  else//i==1 --> c'è "f" in argv[1]
		{
	    if(strstr(argv[1],"p"))//"f" e se c'è anche "p"...
	      sprintf(p,"%c!",c[5]);//compongo la stringa mettendo "e" al posto di %c!
	    else//"f" e "s" oppure "f" da solo (ultimo caso rimasto)
	      sprintf(p,"%c!",c[3]);//compongo la stringa mettendo "a" al posto di %c!
		}
	}
	printf("%s\n", s_format);
/*** TO BE COMPLETED END ***/
